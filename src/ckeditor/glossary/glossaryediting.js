 ...
 *
 * It introduces the `tooltip="url"` attribute in the model which renders to the view as a `<a href="url">` element
...
export default class GlossaryEditing extends Plugin {
	...
	init() {
		...
		// Enable two-step caret movement for `tooltip` attribute.
		const twoStepCaretMovementPlugin = editor.plugins.get( TwoStepCaretMovement );
		twoStepCaretMovementPlugin.registerAttribute( 'tooltip' );

		// Setup highlight over selected link.
		inlineHighlight( editor, 'tooltip', 'span', HIGHLIGHT_CLASS );
			...
			editor.conversion.for( 'downcast' ).attributeToElement( {
				...
				view: ( manualDecoratorName, { writer } ) => {
					if ( manualDecoratorName ) {
						...
						const element = writer.createAttributeElement( 'span', attributes );
	 ...
	 * `tooltip` attribute of the selection and they can type a "clean" (`tooltip`–less) text right away.
	 *
	...
	_enableInsertContentSelectionAttributesFixer() {
		...
		this.listenTo( model, 'insertContent', () => {
			...
			//
			//		                                 ↰
			//		...<$text tooltip="foo">INSERTED[]</$text>
			//
			// If the selection is not "trapped" by the `tooltip` attribute after inserting, there's nothing
			// to fix there.
			...
			// Before insertion:
			//		                       ↰
			//		<$text tooltip="foo">l[]ink</$text>
			//
			// Expected after insertion:
			//		                               ↰
			//		<$text tooltip="foo">lINSERTED[]ink</$text>
			//
			if ( !nodeBefore ) {
				return;
			}

			// Filter out the following case where the selection has the "tooltip" attribute because the
			// gravity is overridden and some text with another attribute (e.g. <b>INSERTED</b>) is inserted:
			//
			// Before insertion:
			//
			//		                       ↱
			//		<$text tooltip="foo">[]link</$text>
			//
			// Expected after insertion:
			//
			//		                                                          ↱
			//		<$text bold="true">INSERTED</$text><$text tooltip="foo">[]link</$text>
			//
			if ( !nodeBefore.hasAttribute( 'tooltip' ) ) {
				return;
			}
			...
			//
			//		                                                             ↰
			//		<$text tooltip="foo">l</$text><$text tooltip="bar">INSERTED[]</$text><$text tooltip="foo">ink</$text>
			//
			if ( nodeAfter && nodeAfter.hasAttribute( 'tooltip' ) ) {
				return;
			}
	...
	_enableClickingAfterLink() {
		...
		this.listenTo( editor.editing.view.document, 'selectionChange', () => {
			...
			const linkRange = findAttributeRange( position, 'tooltip', selection.getAttribute( 'tooltip' ), editor.model );
	 ...
	 *
	 * The purpose of this action is to allow modifying a text without loosing the `tooltip` attribute (and other).
	 *
	 ...
	 * removing a content right after the "tooltip" attribute.
	 *
	 * If so, the selection should not preserve the `tooltip` attribute. However, if
	 * the {@link module:typing/twostepcaretmovement~TwoStepCaretMovement} plugin is active and
	 * the selection has the "tooltip" attribute due to overriden gravity (at the end), the `tooltip` attribute should stay untouched.
	 *
	...
	_handleDeleteContentAfterLink() {
		...
		this.listenTo( model, 'deleteContent', () => {
			...
			const linkRange = findAttributeRange( position, 'tooltip', tooltip, model );

			// Preserve `tooltip` attribute if the selection is in the middle of the link or
...
// All link-related model attributes start with "link". That includes not only "tooltip"
...
function shouldCopyAttributes( model ) {
	...
	// ...or isn't the link.
	if ( !nodeAtFirstPosition.hasAttribute( 'tooltip' ) ) {
		return false;
	}
	...
	const linkRange = findAttributeRange( firstPosition, 'tooltip', nodeAtFirstPosition.getAttribute( 'tooltip' ), model );
