...
import UnTooltipCommand from './untooltipcommand';
...
export default class TooltipEditing extends Plugin {
	...
	init() {
		...
		editor.conversion.for( 'dataDowncast' )
			.attributeToElement( { model: 'linkHref', view: createTooltipElement } );
		...
		editor.commands.add( 'tooltip', new TooltipCommand( editor ) );
		editor.commands.add( 'untooltip', new UnTooltipCommand( editor ) );

		const tooltipDecorators = getLocalizedDecorators( editor.t, normalizeDecorators( editor.config.get( 'tooltip.decorators' ) ) );

		this._enableAutomaticDecorators( tooltipDecorators.filter( item => item.mode === DECORATOR_AUTOMATIC ) );
		this._enableManualDecorators( tooltipDecorators.filter( item => item.mode === DECORATOR_MANUAL ) );
	...
	_enableAutomaticDecorators( automaticDecoratorDefinitions ) {
		...
		const command = editor.commands.get( 'tooltip' );
		const automaticDecorators = command.automaticDecorators;

		// Adds a default decorator for external links.
		if ( editor.config.get( 'tooltip.addTargetToExternalLinks' ) ) {
			automaticDecorators.add( {
				id: 'tooltipIsExternal',
	 ...
	 * {@link module:link/tooltipcommand~tooltipCommand#manualDecorators} collection (a model for manual decorators state).
	 *
			...
			editor.conversion.for( 'downcast' ).attributeToElement( {
				...
				view: ( manualDecoratorName, { writer } ) => {
					if ( manualDecoratorName ) {
						...
						writer.setCustomProperty( 'tooltip', true, element );
	...
	_enableInsertContentSelectionAttributesFixer() {
		...
		this.listenTo( model, 'insertContent', () => {
			...
			model.change( writer => {
				removeTooltipAttributesFromSelection( writer, tooltipCommand.manualDecorators );
			} );
		}, { priority: 'low' } );
	}
	...
	_enableClickingAfterLink() {
		...
		this.listenTo( editor.editing.view.document, 'selectionChange', () => {
			...
			const tooltipRange = findAttributeRange( position, 'linkHref', selection.getAttribute( 'linkHref' ), editor.model );
	...
	_handleDeleteContentAfterLink() {
		...
		this.listenTo( model, 'deleteContent', () => {
			...
			const tooltipRange = findAttributeRange( position, 'linkHref', linkHref, model );
...
function removeTooltipAttributesFromSelection( writer, manualDecorators ) {
...
function shouldCopyAttributes( model ) {
	...
	const tooltipRange = findAttributeRange( firstPosition, 'linkHref', nodeAtFirstPosition.getAttribute( 'linkHref' ), model );
