...
import { isTooltipElement, TOOLTIP_KEYSTROKE } from './utils';
...
export default class TooltipUI extends Plugin {
	...
	_createActionsView() {
		...
		actionsView.bind( 'href' ).to( tooltipCommand, 'value' );
		actionsView.editButtonView.bind( 'isEnabled' ).to( tooltipCommand );
		actionsView.unlinkButtonView.bind( 'isEnabled' ).to( untooltipCommand );
		...
		this.listenTo( actionsView, 'untooltip', () => {
			editor.execute( 'untooltip' );
			this._hideUI();
		} );
		...
		actionsView.keystrokes.set( TOOLTIP_KEYSTROKE, ( data, cancel ) => {
			this._addFormView();
			cancel();
		} );
	...
	_createToolbarTooltipButton() {
		...
		editor.ui.componentFactory.add( 'tooltip', locale => {
			...
			button.keystroke = TOOLTIP_KEYSTROKE;
			button.tooltip = true;
	...
	_addFormView() {
		...
		this.formView.urlInputView.fieldView.element.value = tooltipCommand.value || '';
	}
	 ...
	 */
	_closeFormView() {
		const tooltipCommand = this.editor.commands.get( 'tooltip' );
